...
class PikestaAppTest {
    ...
    fun `should show home screen when startup state is TokenPresent`() = runComposeUiTest {
        // When
        setContent { PikestaApp(uiState = StartupUiState.TokenPresent, authViewModel = authViewModel, homeViewModel = homeViewModel) }
    ...
    fun `should show auth screen when startup state is TokenAbsent`() = runComposeUiTest {
        // When
        setContent { PikestaApp(uiState = StartupUiState.TokenAbsent, authViewModel = authViewModel, homeViewModel = homeViewModel) }
    ...
    fun `should show loading indicator when startup state is Loading`() = runComposeUiTest {
        // When
        setContent { PikestaApp(uiState = StartupUiState.Loading, authViewModel = authViewModel, homeViewModel = homeViewModel) }
